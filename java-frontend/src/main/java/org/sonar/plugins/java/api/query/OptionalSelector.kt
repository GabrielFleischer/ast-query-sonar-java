...
class OptionalSelector<INIT, CUR>(
  start: Root<INIT>,
  current: IRNode<*, out CUR>
) : Selector<INIT, CUR, CUR?>(start, current) {

  fun <TO> apply(op: Operation1to1<in CUR, TO>): OptionalSelector<INIT, TO> =
  ...
  override fun toSingle(): SingleSelector<INIT, CUR?> = orElseNull()
}
