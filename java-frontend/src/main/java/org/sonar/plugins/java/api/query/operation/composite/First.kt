...
fun <INIT, CUR> ManySelector<INIT, CUR>.first(): OptionalSelector<INIT, CUR> =
  aggregateFilter(FirstFunction())

fun <INIT, OUT> OptionalSelector<INIT, out OUT>.orElse(default: OUT): SingleSelector<INIT, OUT> =
  aggregate(FirstOrDefaultFunction(default))

fun <INIT, CUR> OptionalSelector<INIT, CUR>.orElseNull(): SingleSelector<INIT, CUR?> =
  aggregate(FirstOrDefaultFunction(null))

fun <INIT, OUT> ManySelector<INIT, out OUT>.firstOrElse(default: OUT): SingleSelector<INIT, OUT> =
  aggregate(FirstOrDefaultFunction(default))

fun <INIT, CUR> ManySelector<INIT, CUR>.firstOrNull(): SingleSelector<INIT, CUR?> =
  aggregate(FirstOrDefaultFunction(null))
