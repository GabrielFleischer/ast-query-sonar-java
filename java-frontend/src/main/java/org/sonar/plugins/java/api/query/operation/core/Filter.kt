...
fun <INIT, CUR> SingleSelector<INIT, CUR>.filter(predicate: IdentifiedFunction<(CUR) -> Boolean>): OptionalSelector<INIT, CUR> {
  return apply(FilterOperation(predicate))
}

fun <INIT, CUR> OptionalSelector<INIT, CUR>.filter(predicate: IdentifiedFunction<(CUR) -> Boolean>): OptionalSelector<INIT, CUR> {
  return apply(FilterOperation(predicate))
}

fun <INIT, CUR> ManySelector<INIT, CUR>.filter(predicate: IdentifiedFunction<(CUR) -> Boolean>): ManySelector<INIT, CUR> {
  return apply(FilterOperation(predicate))
}

fun <INIT, CUR> SingleSelector<INIT, CUR>.filter(predicate: (CUR) -> Boolean) =
  filter(idFunction(lambda = predicate))

fun <INIT, CUR> OptionalSelector<INIT, CUR>.filter(predicate: (CUR) -> Boolean) =
  filter(idFunction(lambda = predicate))

fun <INIT, CUR> ManySelector<INIT, CUR>.filter(predicate: (CUR) -> Boolean) =
  filter(idFunction(lambda = predicate))
