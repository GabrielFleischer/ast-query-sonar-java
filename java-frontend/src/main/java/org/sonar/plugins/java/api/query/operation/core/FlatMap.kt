...
fun <INIT, FROM, TO> Selector<INIT, FROM, *>.flatMapSeq(mapper: IdFunc<(FROM) -> Sequence<TO>>): ManySelector<INIT, TO> {
  return apply(FlatMapOperation(mapper))
}

fun <INIT, FROM, TO> Selector<INIT, FROM, *>.flatMapSeq(mapper: (FROM) -> Sequence<TO>) =
  flatMapSeq(idFunction(lambda = mapper))

fun <INIT, FROM, TO> Selector<INIT, FROM, *>.flatMap(mapper: IdLambda<(FROM) -> Collection<TO>>) =
  flatMapSeq(IdLambda(mapper.id, mapper.desc) { mapper.function(it).asSequence() })

fun <INIT, FROM, TO> Selector<INIT, FROM, *>.flatMap(mapper: (FROM) -> Collection<TO>) =
