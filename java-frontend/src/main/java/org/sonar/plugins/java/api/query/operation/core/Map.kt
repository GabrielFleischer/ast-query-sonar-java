...
fun <INIT, FROM, TO> SingleSelector<INIT, FROM>.map(mapper: IdentifiedFunction<(FROM) -> TO>): SingleSelector<INIT, TO> {
  return apply(MapOperation(mapper))
}

fun <INIT, FROM, TO> OptionalSelector<INIT, FROM>.map(mapper: IdentifiedFunction<(FROM) -> TO>): OptionalSelector<INIT, TO> {
  return apply(MapOperation(mapper))
}

fun <INIT, FROM, TO> ManySelector<INIT, FROM>.map(mapper: IdentifiedFunction<(FROM) -> TO>): ManySelector<INIT, TO> {
  return apply(MapOperation(mapper))
}

fun <INIT, FROM, TO> SingleSelector<INIT, FROM>.map(mapper: (FROM) -> TO) = map(idFunction(lambda = mapper))
fun <INIT, FROM, TO> OptionalSelector<INIT, FROM>.map(mapper: (FROM) -> TO) = map(idFunction(lambda = mapper))
fun <INIT, FROM, TO> ManySelector<INIT, FROM>.map(mapper: (FROM) -> TO) = map(idFunction(lambda = mapper))
