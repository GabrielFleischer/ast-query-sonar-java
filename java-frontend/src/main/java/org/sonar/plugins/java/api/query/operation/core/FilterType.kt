...
fun <INIT, FROM, TO : FROM & Any> SingleSelector<INIT, FROM>.filterByType(vararg classes: KClass<out TO>): OptionalSelector<INIT, TO> {
  return apply(FilterTypeOperation(classes.toSet()))
}

fun <INIT, FROM, TO : FROM & Any> OptionalSelector<INIT, FROM>.filterByType(vararg classes: KClass<out TO>): OptionalSelector<INIT, TO> {
  return apply(FilterTypeOperation(classes.toSet()))
}

fun <INIT, FROM, TO : FROM & Any> ManySelector<INIT, FROM>.filterByType(vararg classes: KClass<out TO>): ManySelector<INIT, TO> {
  return apply(FilterTypeOperation(classes.toSet()))
}
