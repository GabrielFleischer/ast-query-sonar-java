...
fun <INIT, C> SingleSelector<INIT, C>.consume(reporter: (ExecutionContext, C) -> Unit): SingleSelector<INIT, C> {
  return apply(ConsumeOperation(reporter))
}

fun <INIT, C> OptionalSelector<INIT, C>.consume(reporter: (ExecutionContext, C) -> Unit): OptionalSelector<INIT, C> {
  return apply(ConsumeOperation(reporter))
}

fun <INIT, C> ManySelector<INIT, C>.consume(reporter: (ExecutionContext, C) -> Unit): ManySelector<INIT, C> {
  return apply(ConsumeOperation(reporter))
}
