 ...
 */
package org.sonar.plugins.java.api.query.exec.greedy

import org.sonar.plugins.java.api.query.Query
import org.sonar.plugins.java.api.query.exec.CombineTest
...
class GreedyCombineTest : CombineTest() {
  override fun <IN, OUT> buildQuery(
    start: Root<IN>, end: IRNode<*, out OUT>, transform: Function<List<OUT>, List<OUT>>
  ): Query<IN, List<OUT>> {
    return GreedyBuilder().buildQuery(start, end) { values -> transform.apply(values) }
  }
}
